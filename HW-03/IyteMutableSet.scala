 ...
 * This is mah MutableSet implementation. Since "contains" method
 * had to be fast ( must be below O(n) ) for this homework,
 * I optimized for it. So, add method in this mutable thing
 * is more bearable than immutable one but still it is ... let
 * say unbearable (compared to "contains" method, adding
 * an element to a 100,000-elemented mutable set tooks around 0.7 ms,
 * so you decide if it is OK). But "contains" method is kinda superfast.
